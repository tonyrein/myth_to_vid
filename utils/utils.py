...
def make_ssh_client(hostname):
    ssh_configs = cfg['SSH_CONFIGS'] # raises KeyError if no such section
    host_cfg = ssh_configs[hostname] # raises KeyError if no such option
    user,pword,port = host_cfg.split(',')
    port = int(port)
